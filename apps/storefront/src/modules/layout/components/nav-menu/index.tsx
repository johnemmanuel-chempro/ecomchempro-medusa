...
export default function NavMenu({ categories }: NavMenuProps) {
  ...
  return (
    <div ref={menuRef} className="relative">
        ...
        <div className="flex items-center gap-x-4 justify-around flex-1">
          {navMenuItems.map((item) => (
            <button
              ...
              className={`hover:bg-[#045a9c] duration-200 rounded-md px-2 py-1 cursor-pointer uppercase text-sm ${
      ...
      {productsOpen && (
        <div className="absolute left-0 right-0 top-full z-50 mt-1 rounded-md bg-white text-black shadow-lg">
          <div className="flex items-start gap-x-0 min-h-[240px]">
            ...
            {selectedL1 && level2.length > 0 && (
              <div className="p-2 min-w-[200px] border-l-2 border-indigo-500">
                {level2.map((category) =>
                  renderCategoryItem(category, {
