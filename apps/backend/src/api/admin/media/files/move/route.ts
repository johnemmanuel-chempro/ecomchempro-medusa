...
// Move (cut + paste) a file into a folder
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
    ...
    try {
        ...
        const moved = await service.moveFileToFolder(body.source_id, folder_id)
        return res.json(moved)
    } catch (error) {
        console.log(error)
        return res.status(400).json({
            message: (error as Error).message || "Failed to move file",
        })
    }
}
